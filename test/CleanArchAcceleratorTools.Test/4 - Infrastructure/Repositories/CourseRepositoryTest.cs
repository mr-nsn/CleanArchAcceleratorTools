...
namespace CleanArchAcceleratorTools.Test.Infrastructure.Repositories;
...
public class CourseRepositoryTest
{
    ...
    public CourseRepositoryTest(CourseFixture courseFixture)
    {
    ...
    [Theory(DisplayName = "CourseRepository - Count - Should return the number of courses matching the predicate (sync)")]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    public void Count_ShouldReturnNumberOfCoursesMatchingPredicate(List<Course> courses)
    {
        // Arrange
        _courseFixture.RepositoryImpl.AddRange(courses);
        _courseFixture.Context.SaveChanges();
        ...
        // Act
        var total = _courseFixture.RepositoryImpl.Count(x => true);

        // Assert
        Assert.Equal(courses.Count, total);
    }

    [Theory(DisplayName = "CourseRepository - CountAsync - Should return the number of courses matching the predicate")]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    [InlineAutoDataCustom()]
    public async Task CountAsync_ShouldReturnNumberOfCoursesMatchingPredicate(List<Course> courses)
    {
        // Arrange
        await _courseFixture.RepositoryImpl.AddRangeAndCommitAsync(courses);

        // Act
        var total = await _courseFixture.RepositoryImpl.CountAsync(c => true);
    ...
    public void DynamicSelect_ShouldReturnOnlySelectedFields(List<Course> courses)
    {
        ...
        // Act
        var results = _courseFixture.RepositoryImpl.DynamicSelect(dynamicFilter: default, dynamicSort: default, fields);
    ...
    public async Task DynamicSelectAsync_ShouldReturnsOnlyFieldsPassedAsParameter(List<Course> courses)
    {
        ...
        // Act
        var results = await _courseFixture.RepositoryImpl.DynamicSelectAsync(dynamicFilter: default, dynamicSort: default, fields);
    ...
    public void SearchWithPagination_ShouldReturnPaginatedAndFilteredResults(List<Course> courses)
    {
        ...
        var queryFilter = _courseFixture.GenerateValidQueryFilter();
        ...
        // Assert
        Assert.NotNull(result);
        Assert.Equal(queryFilter.Page, result.Page);
        Assert.Equal(queryFilter.PageSize, result.PageSize);
        Assert.Equal(courses.Count, result.TotalRecords);
        Assert.NotNull(result.Result);
        Assert.True(result.Result.Count <= queryFilter.PageSize);
